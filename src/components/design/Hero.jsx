...
export const BottomLine = () => {
  return (
    <>
      ...
      {/* Transparent Plus icons */}
      <PlusSvg className="hidden absolute top-[54.9375rem] left-[2.1875rem] z-2 opacity-0 pointer-events-none xl:block" />
      <PlusSvg className="hidden absolute top-[54.9375rem] right-[2.1875rem] z-2 opacity-0 pointer-events-none xl:block" />
    </>
  );
};
...
export const BackgroundCircles = ({ parallaxRef }) => {
  ...
  return (
    <div className="absolute -top-[42.375rem] left-1/2 w-[78rem] aspect-square border border-gray-900 rounded-full -translate-x-1/2 md:-top-[38.5rem] xl:-top-[32rem]">
      <Rings />
      ...
      <MouseParallax strength={0.07} parallaxContainerRef={parallaxRef}>
        ...
        <div className="absolute bottom-1/2 left-1/2 w-0.25 h-1/2 origin-bottom rotate-[54deg]">
          <div className={`hidden w-4 h-4 -ml-1 mt-[12.9rem] bg-gradient-to-b from-[#B9AEDF] to-[#1A1A32] rounded-full xl:block transition-transform duration-500 ease-out ${mounted ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`} />
        </div>
